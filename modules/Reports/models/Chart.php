...
abstract class Base_Chart extends Vtiger_Base_Model{
	...
	function generateLink($field, $value) {
		...
		$chartModel = $this->getParent();
		$reportModel = $chartModel->getParent();
		$months = array("فروردين" => "01","ارديبهشت" => "02","خرداد" => "03", "تير" => "04", "مرداد" => "05", "شهريور" => "06","مهر" => "07","آبان" => "08","آذر" => "09","دي" => "10","بهمن" => "11","اسفند" => "12");
		$monthsDays = array(1 => "31",2 => "31",3 => "31", 4 => "31", 5 => "31", 6 => "31",7 => "30",8 => "30",9 => "30",10 => "30",11 => "30",12 => "29");
		...
		if(($dataFieldInfo[4] == 'D' || $dataFieldInfo[4] == 'DT') && !empty($dataFieldInfo[5])) {
			...
			// print_r($dataValue[0]);
			// pr($months[$dataValue[0]]);die;
			if(count($dataValue) > 1) {
				$comparator = 'bw';
				$m=$months[$dataValue[0]];
				$startDate=$dataValue[1]."-".$m."-01";
				$endDate=$dataValue[1]."-".$m."-".$monthsDays[intval($m)];
				...
				$comparator = 'bw';
				$startDate=$dataValue[0]."-01-01";
				$endDate=$dataValue[0]."-12-29";
				$value=$startDate.",".$endDate;
				// if($dataFieldInfo[4] == 'D') {
...
class VerticalbarChart extends Base_Chart {
	...
	function sortReportByMonth($data) {
		...
		$mOrder = array("فروردين" => 0,"ارديبهشت" => 1,"خرداد" => 2, "تير" => 3, "مرداد" => 4, "شهريور" => 5,"مهر" => 6,"آبان" => 7,"آذر" => 8,"دي" => 9,"بهمن" => 10,"اسفند" => 11);
