	...
	public function process(Vtiger_Request $request) {
		...
		foreach ($conditions as $info) {
			...
			foreach ($info['columns'] as $conditionRow) {
				if($conditionRow['comparator']=="between"){
					$dd=explode(",",$conditionRow['value']);
					$ddArr=array();
					foreach ($dd as $d) {
						$t=explode("-",$d);
					...
					$conditions[$i]['columns'][$j]['value']=implode(",",$ddArr);
				}
				else{
					$t=explode("-",$conditionRow['value']);
					if(count($t)==3 && intval($t['0'])>0 && intval($t['1'])>0 && intval($t['2'])>0){
						$conditions[$i]['columns'][$j]['value']=jalToGre($conditionRow['value']);
					}
				}
		...
		if ($executionCondition == '6') {
			...
			if ($workflowScheduleType == Workflow::$SCHEDULED_WEEKLY) {
				...
				$date = jalToGre($request->get('schdate'));
				...
				$aDates=$request->get('schannualdates');
				foreach ($aDates as $key => $aDate) {
					$aDates[$key]=jalToGre($aDate);
				}
				$annualDates = Zend_Json::encode($aDates);
			}
