...
class Vtiger_Util_Helper {
	...
	public static function formatDateDiffInStrings($dateTime, $isUserFormat = FALSE) {
		try{
			...
			//Jweb
					list($dateInUserFormat, $timeInUserFormat,$meridiem) = explode(' ', tr_num($dateTime));
					$dateArr=explode('-',$dateInUserFormat);
					if($dateArr[0]<1800){
							$dateTime=implode("-",jalali_to_gregorian($dateArr[0],$dateArr[1],$dateArr[2]))." ".$timeInUserFormat;
							if($meridiem=="ق.ظ"){
									$meridiem="AM";
							}
							else if($meridiem=="ب.ظ"){
									$meridiem="PM";
							}
							if($meridiem) $dateTime.=" ".$meridiem;
					}
					$currentUser = Users_Record_Model::getCurrentUserModel();
					$dateTime = Vtiger_Datetime_UIType::getDisplayDateTimeValue($dateTime);
	...
	public static function formatDateIntoStrings($date, $time = false) {
		...
		if ($dateInUserFormat == $today) {
			...
			$dateInUserFormat=jalToGre($dateInUserFormat)." ".$timeInUserFormat;
			$date = strtotime($dateInUserFormat);

			$formatedDate = jdate('l', $date) . ' ' . jdate('d', $date) . ' ' . jdate('F', $date);
			if (jdate('Y', $date) != jdate('Y')) {
				$formatedDate .= ', '.jdate('Y', $date);
			}
	...
	public static function formatDateTimeIntoDayString($dateTime, $skipConversion = FALSE) {
		...
		//Jweb
				$dateArr=explode('-',$dateInUserFormat);
		if($dateArr[0]<1800)
			$dateInUserFormat=implode("-",jalali_to_gregorian($dateArr[0],$dateArr[1],$dateArr[2]));
				//Jweb
		...
		$date = strtotime($dateInUserFormat);
		$formatedDate = vtranslate(jdate('l', $date)). ', ' .jdate('d', $date). ' ' .vtranslate(jdate('F', $date)). ' ' .jdate('Y', $date);
		$formatedDate .= ' ' .vtranslate('LBL_ATClock'). ' ' .$displayTime;
		...
		foreach($listSearchParams as $groupInfo){
			...
			foreach($groupInfo as $fieldSearchInfo){
				   ...
				   $fieldValue = $fieldSearchInfo[2];
					...
					if($fieldInfo->getFieldDataType() == "date" || $fieldInfo->getFieldDataType() == "datetime" ) {
										 $valArr=explode(',',$fieldValue);
										 $resArr=array();
										 foreach ($valArr as $vv) {
											 $resArr[]=jalToGre($vv);
										 }
										 $fieldValue=implode(',',$resArr);
									 }
