/* http://keith-wood.name/datepick.html
   ...
   Please attribute the author if you use it. */
	 ...
	 function addMonth(tarikh, addedM) {
	 	month = tarikh[1] + addedM;
	 	year = Math.floor((month - 1) / 12);
	 	tarikh[0] += year;
	 	tarikh[1] = month - (year * 12);
	 	if (tarikh[2] > JalaliDate.j_days_in_month[tarikh[1]]) tarikh[2] = JalaliDate.j_days_in_month[tarikh[1]];
	 	return tarikh;
	 }
	 function addYears(tarikh, n) {
	 	tarikh[0] += n;
	 	return tarikh;
	 };
...
function Datepicker() {
	...
	this.regional[''] = { // US/English
		...
		dayNamesMin: ['یک', 'دو', 'سه', 'چه', 'پن', 'جم', 'شن'],
		dateFormat: 'mm/dd/yyyy', // See options on formatDate
		firstDay: 6, // The first day of the week, Sun = 0, Mon = 1, ...
		renderer: this.defaultRenderer, // The rendering templates
		prevText: '&lt;قبلی', // Text for the previous month command
		prevStatus: 'نمایش ماه قبل', // Status text for the previous month command
		prevJumpText: '&lt;&lt;', // Text for the previous year command
		prevJumpStatus: 'نمایش سال قبل', // Status text for the previous year command
		nextText: 'بعدی&gt;', // Text for the next month command
		...
		currentText: 'ماه جاری', // Text for the current month command
...
$.extend(Datepicker.prototype, {
			...
			get: function(name) { // Get a setting value, computing if necessary
				if ($.inArray(name, ['defaultDate', 'minDate', 'maxDate']) > -1) { // Decode date settings
					if(name=='minDate'){
						...
						var g_y=gCurDate.getFullYear();
						var g_m=gCurDate.getMonth();
						var g_d=gCurDate.getDate();
						var tarikh = geo_to_j(g_y, g_m, g_d);
						var miladi = jal_to_g(tarikh[0],1,1);
						return plugin.newDate(miladi[0],miladi[1],miladi[2]);
					}
					else if(name=='maxDate'){
						var gCurDate=new Date();
						var g_y=gCurDate.getFullYear()+1;
						var g_m=gCurDate.getMonth();
						var g_d=gCurDate.getDate();
						var tarikh = geo_to_j(g_y, g_m, g_d);
						var miladi = jal_to_g(tarikh[0],1,1);
	...
	_generateContent: function(target, inst) {
		...
		var miladi_y = drawDate.getFullYear();
		var miladi_m = drawDate.getMonth();
		var miladi_d = drawDate.getDate();
		var tarikh = geo_to_j(miladi_y, miladi_m + 1, miladi_d);
		// Generate months
		var monthRows = '';
		for (var row = 0; row < monthsToShow[0]; row++) {
			var months = '';
			for (var col = 0; col < monthsToShow[1]; col++) {
				months += this._generateMonth(target, inst, tarikh[0],
					tarikh[1], inst.options.renderer, (row == 0 && col == 0));
					tarikh = addMonth(tarikh, 1);
			}
	...
	_generateMonth: function(target, inst, year, month, renderer, first) {
		var daysInMonth = JalaliDate.j_days_in_month[month-1];//plugin.daysInMonth(year, month);
		var monthsToShow = inst.options.monthsToShow;
		monthsToShow = ($.isArray(monthsToShow) ? monthsToShow : [1, monthsToShow]);
		...
		var jy=year;
		var jm=month;
		var milady = jal_to_g(year,month,1);
		// year=milady[0];
		// month=milady[1];
		var firstDay = inst.options.firstDay;
		var leadDays = (plugin.newDate(milady[0], milady[1], milady[2]).getDay() - firstDay + 7) % 7;
		...
		var drawDate = plugin.newDate(milady[0], milady[1], milady[2]);
		...
		for (var week = 0; week < numWeeks; week++) {
			...
			for (var day = 0; day < 7; day++) {
				...
				var g_y=drawDate.getFullYear();
				var g_m=drawDate.getMonth()+1;
				var g_d=drawDate.getDate();
				var dateCn=geo_to_j(g_y,g_m,g_d);


				var dateInfo = (!$.isFunction(inst.options.onDate) ? {} :
					inst.options.onDate.apply(target, [drawDate, dateCn[1] == month]));
				var selectable = (selectOtherMonths || dateCn[1] == month) &&
					this._isSelectable(target, drawDate, dateInfo.selectable, minDate, maxDate);

				var dateTitle=dateCn.join('-');
				days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
					(selectable ? '<a href="javascript:void(0)"' : '<span') +
					' class="dp' + ts + ' ' + (dateInfo.dateClass || '') +
					(selected && (selectOtherMonths || dateCn[1] == month) ?
					' ' + renderer.selectedClass : '') +
					(selectable ? ' ' + renderer.defaultClass : '') +
					(drawDate.getDay() != 5 ? '' : ' ' + renderer.weekendClass) +
					(dateCn[1] == month ? '' : ' ' + renderer.otherMonthClass) +
					(drawDate.getTime() == today.getTime() && (dateCn[1]) == month ?
					' ' + renderer.todayClass : '') +
					(drawDate.getTime() == inst.drawDate.getTime() && (dateCn[1]) == month ?
					' ' + renderer.highlightedClass : '') + '"' +
					(dateInfo.title || (inst.options.dayStatus && selectable) ? ' title="' +
					(dateInfo.title || dateTitle ) + '"' : '') + '>' +
					(inst.options.showOtherMonths || dateCn[1] == month ?
					dateInfo.content || dateCn[2] : '&nbsp;') +
					(selectable ? '</a>' : '</span>'));
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
		...
		if (!inst.options.changeMonth) {
			return plugin.formatDate(
				monthHeader, plugin.newDate(milady[0], milady[1], milady[2]), inst.getConfig());
		}
		// Months
		...
		for (var m = 1; m <= 12; m++) {
			var milady1=jal_to_g(year,m,JalaliDate.j_days_in_month[m-1]);
			var milady2=jal_to_g(year,m,1);
			if ((!minDate || plugin.newDate(milady1[0], milady1[1], milady1[2]).
					getTime() >= minDate.getTime()) &&
					(!maxDate || plugin.newDate(milady2[0], milady2[1], milady2[2]).getTime() <= maxDate.getTime())) {
