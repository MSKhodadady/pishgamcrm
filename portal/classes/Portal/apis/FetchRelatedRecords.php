 ...
 * All Rights Reserved.
 * ***********************************************************************************/
 include("include/utils/portaljdf.php");
	...
	public function processResponse($result, $relatedModule, $language) {
		...
		if ($relatedModule == 'ModComments') {
			foreach ($result as $key => $valueArray) {
				$result[$key]['modifiedtime']=portalJDate($valueArray['modifiedtime']);
				$result[$key]['createdtime']=portalJDate($valueArray['createdtime']);
			}
			...
			if ($result[$i]) {
				...
				foreach ($result[$i] as $field => $value) {
					...
					if (is_array($value)) {
						...
						if ($recordMeta[$field]['type'] == 'date') {
							$value=portalJDate($value);
						}
