...
function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='en'){
 ...
 list($j_y,$j_m,$j_d)=gregorian_to_jalali($date[8],$date[3],$date[2],'',1);
...
function AmPmSet($date){
  $dateArr=explode(" ",$date);
  if(count($dateArr)==3){
    if(trim($dateArr[2])=="ب.ظ") $dateArr[2]="PM";
    else if(trim($dateArr[2])=="ق.ظ") $dateArr[2]="AM";
  }
...
function portalJDate($date){
  $dt=explode(" ",$date);
  $jd=greToJal($dt[0]);
  $d=explode("-",$jd);
  if(intval($d[1]) > 6 && strlen($dt[1]) > 3){
    $st=strtotime($date)+12600;
    $jd=$jd." ".date("H:i:s",$st);
  }
  else if(strlen($dt[1]) > 3){
    $st=strtotime($date)+16200;
    $jd=$jd." ".date("H:i:s",$st);
  }
  return $jd;
}
/*	F	*/
function jalToGre($date,$mod="-"){
    $dateTimeArr=explode(" ",$date);
  $dateArr=explode($mod,$dateTimeArr[0]);
	if(intval($dateArr[0])<1800 && intval($dateArr[0])>1200){
  		$date = jalali_to_gregorian($dateArr[0],$dateArr[1],$dateArr[2],$mod);
      if(strlen($dateTimeArr[1])>1)
        $date=$date.' '.$dateTimeArr[1];
  }
	return $date;
}
function greToJal($date,$mod="-"){
    $dateTimeArr=explode(" ",$date);
  	$dateArr=explode($mod,$dateTimeArr[0]);
  	if(intval($dateArr[0])>1800){
      $date=gregorian_to_jalali($dateArr[0],$dateArr[1],$dateArr[2],$mod);
      if(strlen($dateTimeArr[1])>1)
        $date=$date.' '.$dateTimeArr[1];
    }
    return $date;
}
function gregorian_to_jalali($g_y,$g_m,$g_d,$mod='',$fnJdate=0){
 ...
 $dateArr=array();
 if($fnJdate==0){
   if(intval($jm)<10) $jm="0".$jm;
   if(intval($jd)<10) $jd="0".$jd;
  }
 if($g_y>0) $dateArr[]=$jy;
 if($g_m>0) $dateArr[]=$jm;
 if($g_d>0) $dateArr[]=$jd;
 if($mod=='') return $dateArr;
 else return implode($mod,$dateArr);
...
function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
 ...
 $dateArr=array();
	if(intval($gm)<10) $gm="0".$gm;
	if(intval($gd)<10) $gd="0".$gd;
 if($j_y>0) $dateArr[]=$gy;
 if($j_m>0) $dateArr[]=$gm;
 if($j_d>0) $dateArr[]=$gd;
 if($mod=='') return $dateArr;
