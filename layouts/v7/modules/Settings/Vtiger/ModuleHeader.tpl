		...
		<div class="module-action-content clearfix">
			<div class="col-lg-7 col-md-7">
				{if $USER_MODEL->isAdminUser()}
					...
					&nbsp;<span class="fa fa-angle-left pull-left {if $VIEW eq 'Index' && $MODULE eq 'Vtiger'} hide {/if}" aria-hidden="true" style="padding-top: 12px;padding-left: 5px; padding-right: 5px;"></span>
				{/if}
					...
					{if $MODULE neq 'Vtiger'}
						...
						<span class="current-filter-name settingModuleName filter-name pull-left">
							{if $smarty.request.view eq 'Calendar'}
								...
								{else}
									{vtranslate({$PAGETITLE}, $QUALIFIED_MODULE)}&nbsp;<span class="fa fa-angle-left" aria-hidden="true"></span>&nbsp;{$USER_MODEL->getName()}
								{/if}
								...
								<li>
									<button class="btn btn-default addButton module-buttons"
										...
										{if $MODULE eq 'Tags'}
											...
											<span class="hidden-sm hidden-xs">&nbsp;&nbsp;{vtranslate('LBL_ADD_TAG', $QUALIFIED_MODULE)}</span>
										{else}
											{if $LISTVIEW_BASICACTION->getIcon()}
												<i class="{$LISTVIEW_BASICACTION->getIcon()}"></i>
											{/if}

											<span class="hidden-sm hidden-xs">&nbsp;&nbsp;{vtranslate($LISTVIEW_BASICACTION->getLabel(), $QUALIFIED_MODULE)}</span>
										{/if}
									</button>
								</li>
							{/foreach}
