...
Settings_Vtiger_Edit_Js("Settings_Workflows_Edit_Js", {
   ...
   registerCcAndBccEvents: function () {
	...
	updateAnnualDates : function(annualDatesEle) {
        ...
        var mDate=annualDates[j].split('-');
        var tarikh = geo_to_j(mDate[0],mDate[1],mDate[2]);
        tarikh[0]=String(tarikh[0]);
        tarikh[1]=String(tarikh[1]);
        tarikh[2]=String(tarikh[2]);
        if (tarikh[1].length == 1) {
            tarikh[1] = "0" + tarikh[1];
        }
        if (tarikh[2].length == 1) {
            tarikh[2] = "0" + tarikh[2];
        }
        var jannualDate = tarikh.join('-');
				hiddenDates += '<option selected value='+jannualDate+'>'+jannualDate+'</option>';
			}
	...
	DateToYMD : function (date) {
        ...
        var tarikh=geo_to_j(year,month,day);
        tarikh[0]=String(tarikh[0]);
        tarikh[1]=String(tarikh[1]);
        tarikh[2]=String(tarikh[2]);
        if (tarikh[1].length == 1) {
        ...
        return tarikh[0] + "-" + tarikh[1] + "-" + tarikh[2];
    },
