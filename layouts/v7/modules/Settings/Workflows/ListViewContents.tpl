						...
						{foreach item=MODULE_MODEL key=TAB_ID from=$SUPPORTED_MODULE_MODELS}
							<option {if $SOURCE_MODULE eq $MODULE_MODEL->getName()} selected="" {/if} value="{$MODULE_MODEL->getName()}" data-count='{if $MODULES_COUNT[$TAB_ID]}{$MODULES_COUNT[$TAB_ID]}{else}0{/if}'>
								{if $MODULE_MODEL->getName() eq 'Calendar'}
									{vtranslate('LBL_WORKFLOWS')}&nbsp;{vtranslate('LBL_TASK', $MODULE_MODEL->getName())}
								{else}
									{vtranslate('LBL_WORKFLOWS')}&nbsp;{vtranslate($MODULE_MODEL->getName(),$MODULE_MODEL->getName())}
								{/if}
							</option>
						{/foreach}
					</select>
							...
							{if $LISTVIEW_ENTRIES_COUNT eq '0'}
								<tr class="emptyRecordsDiv">
									...
									<td colspan="{$COLSPAN_WIDTH}" style="vertical-align:inherit !important;">
										...
										<center>{vtranslate('LBL_NOTHING')} {vtranslate($SINGLE_MODULE, $QUALIFIED_MODULE)}{vtranslate('LBL_PY')} {vtranslate('LBL_NOT_FOUND')}</center>
									</td>
								</tr>
							{/if}
						</tbody>
