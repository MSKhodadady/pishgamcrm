		...
		<tbody>
			<tr>
				<td>
					...
					{vtranslate('LBL_NOTHING')} {vtranslate('ProjectTask', 'ProjectTask')}{vtranslate('LBL_PY')} {vtranslate('LBL_NOT_FOUND')} {vtranslate('LBL_NO_DATE_VALUE_MSG', 'ProjectTask')}.{if $IS_MODULE_EDITABLE} <a href="{$PROJECT_TASK_MODEL->getCreateRecordUrl()}&projectid={$PARENT_ID}"> {vtranslate('LBL_CREATE')} </a>{/if}
				</td>
			</tr>
		</tbody>
	</table>
