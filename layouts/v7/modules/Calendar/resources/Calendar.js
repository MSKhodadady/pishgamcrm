...
//since support for multiple calendar views for events is enabled
	...
	setStartDateTime: function (modalContainer, startDateTime) {
		...
		startDateElement.val(moment(startDateTime.format(vtUtils.getMomentDateFormat()),vtUtils.getMomentDateFormat()).format(vtUtils.getJMomentDateFormat()));
	...
	registerPopoverEvent: function (event, element, calendarView) {
		...
		var generatePopoverContentHTML = function (eventObj) {
			...
			if (eventObj.activitytype === 'Task') {
				...
				jdateFormat = app.convertToDatePickerJformat(dateFormat.toLowerCase());
				temp = moment(moment(eventObj._start.format(dateFormat),vtUtils.getMomentDateFormat()));
				timeString = temp.format(jdateFormat);
			}
		...
		agendaListView = view.extend({
			initialize: function () {
				var dateFormat = thisInstance.getUserPrefered('date_format');
					this.vtDateFormat = app.convertToDatePickerJformat(dateFormat);
			},
			getCourseDay: function (date) {
				var today = moment();
				var dateFormat = this.vtDateFormat;
					date = moment(moment(date.format(dateFormat),vtUtils.getMomentDateFormat()));
				...
				return date.format('jMMMM jDD jYYYY');
			},
...
//cleanup before render
	...
	registerGotoDateButtonAction: function (navigationsContainer) {
		...
		gotoButton.daterangepicker({
		    ...
		    locale: {
		        separator: ',',
		        format: app.convertToDatePickerJformat(thisInstance.getUserPrefered('date_format'))
            }
			}).on('apply.daterangepicker', function (e,picker) {
				thisInstance.getCalendarViewContainer().fullCalendar('gotoDate', picker.startDate);
			});
		//
	},
