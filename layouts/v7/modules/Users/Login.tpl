				...
				<div id="forgotPasswordDiv" class="hide">
					<form class="form-horizontal" action="forgotPassword.php" method="POST">
						...
						<div class="group">
							...
							<span><a class="forgotPasswordLink pull-right" style="color: #15c;">بازگشت</a></span>
						</div>
					</form>
				</div>
			</div>
		...
		<script>
			jQuery(document).ready(function () {
				...
				loginFormDiv.find('button').on('click', function () {
					...
					var errorMessage = '';
					if (username === '') {
						errorMessage = 'لطفا نام کاربری خود را وارد نمایید';
						result = false;
					} else if (password === '') {
						errorMessage = 'لطفا رمزعبور خود را وارد نمایید';
						result = false;
					}
					if (errorMessage) {
				...
				forgotPasswordDiv.find('button').on('click', function () {
					...
					if (username === '') {
						...
						errorMessage = 'لطفا ایمیل خود را به صورت صحیح وارد نمایید';
						result = false;
					} else if (email.match(illegalChars)) {
						errorMessage = 'ایمیل وارد شده صحیح نمی باشد.';
						result = false;
					}
					if (errorMessage) {
						validationMessage.removeClass('hide').text(errorMessage);
