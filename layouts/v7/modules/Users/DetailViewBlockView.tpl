								...
								{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
									...
									{if $FIELD_MODEL->get('uitype') eq "83"}
										{foreach item=tax key=count from=$TAXCLASS_DETAILS}
											...
											{/if}
											<div class="fieldLabel col-xs-6 col-md-3 {$WIDTHTYPE}">
									...
									{else}
										...
										{else}
											{assign var=COUNTER value=$COUNTER+1}
										{/if}
										<div class="fieldLabel col-xs-6 col-md-3 {$WIDTHTYPE} textOverflowEllipsis" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
											<span class="muted">
