								...
								<div class="emptyRecordsContent">
									<center>
										{if $SEARCH_VALUE eq 'Active'}
											...
											{vtranslate('LBL_NOTHING')} {vtranslate($SINGLE_MODULE, $MODULE)}{vtranslate('LBL_PY')} {vtranslate('LBL_NOT_FOUND')}.,{if $IS_MODULE_EDITABLE} <a style="color:blue" href="{$MODULE_MODEL->getCreateRecordUrl()}"> {vtranslate('LBL_CREATE_USER',$MODULE)}</a>{/if}
										{else}
											{assign var=SINGLE_MODULE value="SINGLE_$MODULE"}
											{vtranslate('LBL_NOTHING')} {vtranslate($SINGLE_MODULE, $MODULE)}{vtranslate('LBL_PY')} {vtranslate('LBL_NOT_FOUND')}
										{/if}
									</center>
								</div>
							</td>
