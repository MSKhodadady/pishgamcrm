							...
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{if $FIELD_MODEL->get('uitype') eq "83"}
									{foreach item=tax key=count from=$TAXCLASS_DETAILS}
										...
										{/if}
										<div class="fieldLabel {$WIDTHTYPE} col-xs-6 col-md-3 medium">
											<span class='muted'>{vtranslate($tax.taxlabel, $MODULE)}(%)</span>
										</div>
										<divclass="fieldValue {$WIDTHTYPE} col-xs-6 col-md-3 medium">
								...
								{else if $FIELD_MODEL->get('uitype') eq "69" || $FIELD_MODEL->get('uitype') eq "105"}
									{if $COUNTER neq 0}
										...
										{/if}
									{/if}
									<div class="fieldLabel {$WIDTHTYPE}  col-xs-6 col-md-3 medium"><span class="muted">{vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}</span></div>
									<div class="fieldValue {$WIDTHTYPE}  col-xs-6 col-md-3 medium">
								...
								{else}
									{if $FIELD_MODEL->get('uitype') eq "20" or $FIELD_MODEL->get('uitype') eq "19" or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'}
										{if $COUNTER eq '1'}
											<div class="fieldLabel {$WIDTHTYPE} col-xs-6 col-md-3 medium"></div><div class="{$WIDTHTYPE}"></div></div><div class="row">
											{assign var=COUNTER value=0}
										{/if}
									{/if}
									{if $COUNTER eq 2}
									...
									{else}
										{assign var=COUNTER value=$COUNTER+1}
									{/if}
									<div class="fieldLabel textOverflowEllipsis {$WIDTHTYPE} col-xs-6 col-md-3 medium" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
										<span class="muted">
									...
									<div class="fieldValue {$WIDTHTYPE} col-xs-6 col-md-3 medium" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
								...
								{if $FIELD_MODEL_LIST|@count eq 1 and $FIELD_MODEL->get('uitype') neq "19" and $FIELD_MODEL->get('uitype') neq "20" and $FIELD_MODEL->get('uitype') neq "30" and $FIELD_MODEL->get('name') neq "recurringtype" and $FIELD_MODEL->get('uitype') neq "69" and $FIELD_MODEL->get('uitype') neq "105"}
									<div class="fieldLabel {$WIDTHTYPE} col-xs-6 col-md-3 medium"></div><div class="{$WIDTHTYPE}"></div>
								{/if}
							{/foreach}
