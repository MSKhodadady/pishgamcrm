    ...
    <div class="col-lg-6 detailViewButtoncontainer">
        <div class="pull-right btn-toolbar">
            ...
            {if $MODULE_MODEL->isStarredEnabled()}
                <button class="hidden-sm hidden-xs btn btn-default markStar {if $STARRED} active {/if}" id="starToggle" style="width:100px;">
            ...
            {foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
                <button class="btn btn-default" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
                        ...
                        {/if}>
                        {assign var=LINKICON value=Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}
                        <i class="fa {$LINKICON}"></i><span class="hidden-sm hidden-xs">&nbsp;&nbsp;
                    {vtranslate($DETAIL_VIEW_BASIC_LINK->getLabel(), $MODULE_NAME)}</span>
                </button>
            {/foreach}
            {if $DETAILVIEW_LINKS['DETAILVIEW']|@count gt 0}
                <button class="btn btn-default dropdown-toggle" data-toggle="dropdown" href="javascript:void(0);">
                  <i class="fa fa-th-list"></i><span class="hidden-sm hidden-xs">&nbsp;&nbsp;
                   {vtranslate('LBL_MORE', $MODULE_NAME)}</span>&nbsp;&nbsp;<i class="caret"></i>
                </button>
