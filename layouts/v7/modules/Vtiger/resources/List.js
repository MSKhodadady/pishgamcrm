	...
	registerDeleteRecordClickEvent: function () {
		...
		jQuery('#page').on('click', '.deleteRecordButton', function (e) {
			...
			if (typeof recordId === 'undefined') {
					recordId= parent.closest('tr').data('id');
			}
...
//
	...
	registerDropdownPosition: function (container) {
		...
		jQuery('.table-actions').on('click', '.dropdown', function (e) {
			if(clickId!=''){
				jQuery('ul.dropdown-menu.'+clickId).css('display','none');
			}
			...
			clickId="drpdn"+dropdown.closest("tr").data('id');
			jQuery('ul.dropdown-menu.'+clickId).css('display','block');
			...
			if (container === "#page") {
				...
				currtargetRight = jQuery(window).width()-dropdown.offset().left-17;
			...
			if (windowBottom < 250) {
				ftop = 'auto';
				fbottom = (dropdownBottom-5) + 'px';
			}
			else {
				ftop = (currtargetTop-5) + 'px';
				fbottom = "auto";
			}
				...
				fixed_dropdown_menu.css({
					...
					'top': ftop,
					'left': 'auto',
					'right': currtargetRight + 'px',
					'bottom': fbottom
				}).appendTo(containerTarget);
				fixed_dropdown_menu.addClass("clicked");
				fixed_dropdown_menu.addClass(clickId);
		...
		jQuery('.listViewEntries').mouseleave(function (e) {
			...
			setTimeout(function () {
				if (jQuery('.dropdown-menu:hover').length == 0) {
					jQuery('ul.dropdown-menu.clicked').css('display','none');
