...
class Vtiger_PDF_InventoryContentViewer extends Vtiger_PDF_ContentViewer {
	...
	function initDisplay($parent) {
		...
		foreach($this->cells as $cellName => $cellWidth) {
			...
			$pdf->MultiCell($cellWidth, $this->headerRowHeight, $cellLabel, 1, 'C', 1, 1, $contentFrame->x+$offsetX, $contentFrame->y);
			$offsetX += $cellWidth;
		}
		$pdf->SetFont('','');
		// Reset the y to use
	...
	function displayPreLastPage($parent) {
		...
		for ($index = 0; $index < $totalModels; ++$index) {
			...
			$offsetX = 0;
			foreach($this->cells as $cellName => $cellWidth) {
				$pdf->MultiCell($cellWidth, $contentHeight, $model->get($cellName), 0, 'C', 0, 1, $contentLineX+$offsetX, $contentLineY);
				$offsetX += $cellWidth;
			}
		...
		if ($this->contentSummaryModel) {
			...
			foreach($summaryCellKeys as $key) {
				...
				$pdf->MultiCell($contentFrame->w-$summaryLineX+10-$summaryCellLabelWidth, $summaryCellHeight,
					$this->contentSummaryModel->get($key), 1, 'C', 0, 1, $summaryLineX+$summaryCellLabelWidth, $summaryLineY);
