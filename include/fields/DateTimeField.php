...
class DateTimeField {
	...
	public static function __convertToUserFormat($date, $format) {
		...
		(($_POST['action']=='Save' || $_POST['action']=='SaveAjax') ||
		($_POST['module']=='Calendar' && $_POST['action']=='Feed'))){
			$dateArr=explode('-',tr_num($date[0]));
			if($dateArr[0]<1800)
				$date[0]=jalali_to_gregorian($dateArr[0],$dateArr[1],$dateArr[2],"-");
			else
				$date[0]=$date[0];
		}
		else{
			$dateArr=explode('-',tr_num($date[0]));
			if($dateArr[0]>1800)
				$date[0]=gregorian_to_jalali($dateArr[0],$dateArr[1],$dateArr[2],"-");
			// else if($dateArr[0]<1800)
			// 	$date[0]=jalali_to_gregorian($dateArr[0],$dateArr[1],$dateArr[2],"-");
		}
		if ($date[1] != '') {
	...
	function getDisplayTime( $user = null ) {
		...
		$this->datetime=jalToGre($this->datetime);
		$date = self::convertToUserTimeZone($this->datetime, $user);
