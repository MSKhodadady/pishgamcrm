...
class Vtiger_InventoryPDFController {
	...
	function buildHeaderModel() {
		...
		$headerModel->set('moduleTitle', $translatedSingularModuleLabel);
	...
	function buildHeaderModelColumnLeft() {
		...
		if($num_rows) {
			...
			$modelColumnLeft['info']['address'] = $resultrow['address'];
			if(!empty($resultrow['city'])) $modelColumnLeft['info']['city']= $resultrow['city'];
			if(!empty($resultrow['state'])) $modelColumnLeft['info']['state']= $resultrow['state'];
			if(!empty($resultrow['code'])) $modelColumnLeft['info']['code']= $resultrow['code'];
			if(!empty($resultrow['country'])) $modelColumnLeft['info']['country']= $resultrow['country'];

			if(!empty($resultrow['phone']))		$modelColumnLeft['info']['phone']= getTranslatedString("Phone: ", $this->moduleName). $resultrow['phone'];
			if(!empty($resultrow['fax']))		$modelColumnLeft['info']['fax']= getTranslatedString("Fax: ", $this->moduleName). $resultrow['fax'];
			if(!empty($resultrow['website']))	$modelColumnLeft['info']['website']= getTranslatedString("Website: ", $this->moduleName). $resultrow['website'];
      if(!empty($resultrow['vatid']))  $modelColumnLeft['info']['vatid']= getTranslatedString("VAT ID: ", $this->moduleName). $resultrow['vatid'];
	...
	function buildHeaderBillingAddress() {
		...
		if(!empty($billCountry))
			$address .= $billCountry."، ";
		$address = $this->joinValues(array($billState,$billCity), '، ')."، ";
		$address .= $this->joinValues(array($billPoBox, $billStreet), ' ')."     کدپستی: ".$billCode;
		return $address;
	}
	...
	function buildHeaderShippingAddress() {
		...
		$address .= $shipCountry."، ";
		$address .= $this->joinValues(array($shipState,$shipCity), '، ');
		$address	= $this->joinValues(array($shipPoBox, $shipStreet), ' ')."     کدپستی: ".$shipCode;
		return $address;
	}
