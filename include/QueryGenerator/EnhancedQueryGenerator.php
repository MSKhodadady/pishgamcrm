			...
			if (count($filtercolumns) > 0) {
				...
				foreach ($filtercolumns as $index => $filter) {
					...
					if (($nameComponents[4] == 'D' || $nameComponents[4] == 'DT') && in_array($filter['comparator'], $dateSpecificConditions)) {
						...
						$value[] = jalToGre($startDate);
						$value[] = jalToGre($endDate);
						...
						for ($i = 0; $i < count($value); $i++) {
							...
							$value[$i] = jalToGre($values[0]).' '.$values[1];
						}
						...
						if ($nameComponents[4] == 'DT' || $nameComponents[4] == 'D'){
							...
							if(count($arr)>1){
								...
								$filter['value']=implode(",",$cArr);
							}
							else{
								$filter['value']=jalToGre($filter['value']);
							}
						}
