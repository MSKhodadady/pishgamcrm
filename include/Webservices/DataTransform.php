	...
	class DataTransform{
		...
		function filterAndSanitize($row,$meta){
			...
			foreach ($row as $k => $v) {
				$vs=explode(' ',$v);
				$vd=explode('-',$vs['0']);
				if(count($vd)==3 && intval($vd['0']) > 0 && intval($vd['1']) > 0 && intval($vd['2']) > 0){
					if(count(explode(':',$vs['1']))==3)
						$row[$k]=jdate("Y-m-d H:i:s", strtotime($v));
					else if(empty(trim($vs['1'])))
						$row[$k]=jdate("Y-m-d", strtotime($v));
				}
			}
